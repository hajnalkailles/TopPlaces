...
// how often we fetch new photos if we are in the foreground (in seconds)
        ...
        NSURLSessionDownloadTask *task = [session downloadTaskWithRequest:request
            completionHandler:^(NSURL *localFile, NSURLResponse *response, NSError *error) {
                if (error) {
                    ...
                    [self loadFlickrPhotosFromLocalURL:localFile intoContext:self.databaseContext andThenExecuteBlock:^{ completionHandler(UIBackgroundFetchResultNewData);
                                            }
                    ];
                }
            }];
